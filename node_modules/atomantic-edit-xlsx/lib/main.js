...
EditXlsx = (function() {
  ...
  function EditXlsx(xlsxPath) {
    ...
    for (filepath in _ref) {
      ...
      mkdirp.sync(path.dirname(obj.name));
  ...
  EditXlsx.prototype.changeImage = function(file, buffer) {
    process.chdir(path.join(this.tempDir, "xl/media/"));
    fs.writeFileSync(file, buffer);
    process.chdir(this.cwd);
  };

  EditXlsx.prototype.saveToBuffer = function(opts) {
    var buffer, data, i, sheet, zip, _i, _j, _len, _ref, _ref1;
    ...
    this.files.forEach(function(path) {
      return zip.file(path, fs.readFileSync(path));
    });
    process.chdir(this.cwd);
    data = zip.generate({
      type: 'arraybuffer'
